...
public class TSIClassifier : IClassificationStep
{
    ...
    async Task<HttpResponseData?> IClassificationStep.ExecuteAsync(TicketClassificationContext context)
    {
        ...
        {
            _logger.LogError(ex, "TSI Classification failed - TicketId: {TicketId}, BoardId: {BoardId}, BoardName: {BoardName}, ExceptionType: {ExceptionType}, Message: {Message}", 
                context.TicketRequest.TicketId, context.BoardResponse?.BoardId ?? 0, context.BoardResponse?.BoardName ?? "Unknown", ex.GetType().Name, ex.Message);
            
            // Log the failure but continue processing - ticket will be updated with board assignment and summary, but without TSI fields
            var failureLogData = new Dictionary<string, object?>
            {
            ...
            await _loggingService.LogDecisionAsync("TSI Classification", context.TicketRequest.TicketId, 
                context.TicketRequest.CompanyName ?? "Unknown", failureLogData);
            
            // TsiResponse remains null, which is fine - TicketUpdateBuilder handles null TSI responses
        }
